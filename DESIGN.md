...
**검증 상태.** 컴파일·구조 검사·단위 테스트 32건 통과. 실기기 검증 완료 (2026-09-04~06, `README.md` 매트릭스). 진단 패널 3개 탭의 가독성과 Development 계측 상태는 3차 녹화에서 확인했고, 진단 코드가 없는 Production 영상으로 최종 사용자 경로의 시작 시간과 화면 연속성을 교차 확인했다.
...
| 콜백 처리 시간 p50 / p95 / max | p95 <25ms, max <100ms | 누적 11.9 / 17.5 / 22.2ms — 예산 충족 | iPhone 15 Pro · 26.6 · 2026-09-04 |
...
- 1차 누적 통계는 초반 3초 p50 0.31ms에서 말미 11.9ms로 변했다. 이후 3초 고정 창으로 다시 측정하니 p50 12~14ms로 시간 경과 저하 없이 평탄했다. 100ms 처리 주기 대비 큐 점유는 약 12%이고 콜백은 전용 직렬 큐에서 돌아 UI 60fps를 유지한다. 내부 수치는 `SCAN_DIAGNOSTICS`가 있는 Development에서 수집하고, Production은 계측 코드가 없는 최종 사용자 경로의 시작 시간·화면 연속성을 판독하는 역할로 분리했다.
...
**3차 녹화 판독 (2026-09-06, 진단 패널 도입 후 Development·Production 각 1편).** 같은 방·같은 동선으로
스킴당 화면 녹화 1편(Development 139초, Production 124초)을 1fps 프레임 추출 + 패널 텍스트 판독 +
60fps 원본 중복 프레임 계산으로 읽었다. 시나리오: 대기 → 시작 → 걷기 → 렌즈 가림 → 격자 비교(Dev) →
전체화면 측정 → 초기화 (→ Dev만 재시작·일시정지·부하 탭).

| 항목 | Development (Debug + LLDB + 진단) | Production (Release) | 해석 |
| --- | --- | --- | --- |
| 시작 → 첫 격자·mesh | 0.6초 (격자 11,305점 + mesh v1 동시 도착) | ≤2초 (1fps 판독 한계) | R2-1 실시간 갱신 |
| 격자 UI 수신 지연 | 항상 0.0초 전 (매 처리 프레임 발행) | 화면상 실시간 누적 확인 | 스냅샷 경로에 적체 없음 |
| mesh 주기 · 지연 | 3초당 버전 +2 = 1.5초 주기 정확, 원본 프레임 → UI 도착 0.1초 | 화면상 주기 갱신 확인 | 타이머·hop 설계값 일치 |
| 유효점 → 격자 반영 | 2,500~3,072점 / 85~100% (먼 벽·천장 조준 시 126~800점) | 같은 동선의 누적 형태 일치 | stride 4 상한 도달 |
| 연속 누적 최장 | 71.3초 (시작 → 가림) | 전체 걷기 구간 누적 유지 | 가림 전까지 gate 개방 유지 |
| 렌즈 가림 9초 / 7초 | 누적 고정·유효 0점·gate `트래킹 보류`, 복귀 후 유령 벽·궤적 스파이크 없음 | 누적 고정, 복귀 후 미니맵 정합 유지 | normal 게이트 실증 |
| 전체화면(자동 일시정지) | mesh v64 고정, UI 수신 age 0.6 → 6.5초 증가 = 재빌드 스킵 | pts 고정, 관측 면적 17.0 m², 3D 뷰 정상 | 스킵 실증 |
| 초기화 → 재시작 | 세대 0 → 1, 잔상 0, 재시작 0.8초 뒤 첫 격자·1.1초 뒤 mesh | 미니맵 즉시 비움 | 세대 표기로 잔상 구분 가능 |
| 걷기 구간 고유 프레임 비율 (60fps 원본) | 96.0% | 99.8% | Dev 약 4% 중복 = 드롭 또는 정지 장면. Prod 드롭 흔적 없음 |
| 화면 연속성 | DisplayLink 60.0Hz / 최대 간격 16.7ms | 걷기 구간 고유 프레임 99.8% | 내부 계측과 최종 화면을 각각 확인 |
| 콜백 3초 창 p50 / p95 | 17.0 / 19.0ms (세대 1 스캔 마지막 3초) | 최종 경로에서 진단 제외 | 09-05 재측정(12~14 / 13~16ms)과 보충 영상 수치는 아래 기록 |
| 메모리 | 448~452MB, 앱 실행 피크 593.6MB | 최종 경로에서 진단 제외 | 보충 영상의 스캔 중 수치는 아래 기록 |
| 발열 | `높음`(serious) at 128초 | 진단 패널 없음 | 연속 촬영·Debug 계측 조건을 포함한 Development 상태 |

측정 범위: 동일 동선 비교 영상에서는 Development 부하 탭을 일시정지 뒤에 열었고, 아래 보충 영상에서 스캔 중
p50·p95·메모리를 추가 확인했다. 가림은 `insufficientFeatures` → `normal` 직행이어서 3차 영상에서
`재인식 안정화` gate를 재현하지 않았지만, 해당 gate는 09-04 백그라운드 복귀 시나리오에서 별도로 검증됐다.
Production은 패널이 없는 최종 빌드의 픽셀 연속성과 반응 시간을 판독했다.
