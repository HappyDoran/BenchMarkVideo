...
Release 최적화 코드의 내부 수치가 필요할 때 명령줄 조건만으로 같은 계측을 재사용하기 위해서다 (3차 판독에서 실제로 사용).
...
**목표.** UI 30fps 이상(R3-1), 내부 목표는 기기 기본 60fps 유지. 100ms 처리 주기에서 큐 포화를 막기 위해 콜백 최대 <100ms, 안정 여유를 위해 p95 <25ms. 3분 이상 연속 스캔에서 앱 소유 격자·궤적 메모리 상한 유지.
...
5. Release 최적화 코드의 내부 수치는 명령줄 `SWIFT_ACTIVE_COMPILATION_CONDITIONS='$(inherited) SCAN_DIAGNOSTICS'`로 Production scheme을 빌드·설치해 같은 패널로 읽는다 (`README.md` 검증 절). pbxproj의 Release 구성은 그대로 두므로 제출 빌드에는 계측이 없다.
...
| 콜백 처리 시간 p50 / p95 / max | p95 <25ms, max <100ms | Debug 누적 11.9 / 17.5 / 22.2ms — 예산 충족. **Release 계측 1.9 / 2.8 / 7.1ms (09-06 3차)** | iPhone 15 Pro · 26.6 · 2026-09-04 / 09-06 |
...
| 연속 스캔 시간 | ≥3분, 강제 종료 없음 | 3분+ 생존, 메모리 경고 없음. **3차 Release 205초 · Debug 210초 세션 (09-06)** | 〃 / 09-06 |

**해석 (측정 후 추가).**

- 1차 누적 통계는 초반 3초 p50 0.31ms에서 말미 11.9ms로 변했다. 이후 3초 고정 창으로 다시 측정하니 p50 12~14ms로 시간 경과 저하 없이 평탄했다. 100ms 처리 주기 대비 큐 점유는 약 12%이고 콜백은 전용 직렬 큐에서 돌아 UI 60fps를 유지한다. 내부 수치는 `SCAN_DIAGNOSTICS` 계측 빌드에서 수집한다 — Debug와, 명령줄 조건으로 만든 Release 계측 빌드(3차, 아래). 계측 코드가 없는 Production 제출 빌드는 같은 `-O` 코드에서 패널만 빠진 상태다. Release 계측에서 콜백 p50 1.9ms·p95 2.8ms로, Debug 수치의 약 1/12이다.
...
**3차 Release·Debug 계측 판독 (2026-09-06, 각 1편 약 3.5분).** 같은 방·같은 동선으로 Release + `SCAN_DIAGNOSTICS`
계측 빌드(`ScreenRecording_09-06-2026 10-19-00_1.MP4`, 205초)와 Development Debug(`…10-29-56_1.MP4`, 210초)를
부하 탭을 연 채 촬영했다. 1fps 프레임 추출 → 패널 영역 OCR(tesseract kor+eng) → 범위 필터 후 집계. 시나리오:
대기 → 시작 → 3분 이상 걷기 → 갱신 탭 확인 → 일시정지. 두 편을 쉬지 않고 이어서 촬영했다.

| 항목 | Release(-O) + 진단 | Debug(-Onone) + 진단 | 해석 |
| --- | --- | --- | --- |
| 연속 스캔 세션 | **205초**, 강제 종료·메모리 경고·트래킹 경고 없음 | **210초**, 동일 | R3-2 3분 이상 연속 스캔 — 두 빌드 모두 통과 |
| 시작 → 첫 격자 | ≤1초 (녹화 6.6초 `시작 전` → 7.8초 `누적 허용` → 8.4초 29,323점·290셀) | ≤1초 | R2-1 실시간 갱신. 1fps 판독 한계 |
| 콜백 3초 창 p50 (중앙값 / 범위) | **1.9ms** / 1.0~3.6 | 23.4ms / 1.8~40.5 | 최적화만으로 약 12배. Release는 예산 p95 <25ms에 10배 여유 |
| 콜백 3초 창 p95 (중앙값 / 최대) | **2.8ms** / 7.1 | 28.1ms / 50.1 | Debug는 발열 `높음` 상태에서 p95 예산을 넘는다 (아래) |
| 콜백 0.5초 창 평균 / 최대 | 2.0 / 5.6ms | 22.7 / 67.2ms | 두 빌드 모두 max <100ms 큐 안전 조건 충족 |
| mesh CPU 빌드 (중앙값 / 최대) | 7.3 / 13.9ms | 113 / 282ms | 1.5초 주기 대비 Release 0.5%, Debug 7.5% 점유 |
| 처리율 | 0.5초 창 수신 18 / 처리 6 | 15 / 5 | 100ms 스로틀 설계값 그대로 |
| 격자 UI 수신 지연 | 0.0초 전 (매 처리 프레임 발행) | 0.0~0.1초 전 | 스냅샷 경로에 적체 없음 |
| 프레임당 유효점 → 격자 | 2,598~2,661 / 100% | 2,422~2,531 / 100% | stride 4 상한 근접, 깊이 소스 smoothed |
| 누적 (녹화 끝) | 4,156,110점 · 3,652셀 · 깊이 처리 1,857프레임 | 4,539,001점 · 3,571셀 · 1,923프레임 | 같은 동선, 같은 규모 |
| 메모리 footprint (시작 → 끝 / 피크) | 388 → 590MB / 594.6MB | 400 → 608MB / 617.3MB | 증가분은 ARKit mesh 앵커·복셀 색. 빌드 무관하게 같은 기울기 |
| DisplayLink (중앙값 / 최저) | **60.0Hz** / 59.0 | 59.7Hz / 58.3 | Release는 최대 간격 16.7ms 상시. 예외 3구간뿐 (아래) |
| DisplayLink 최대 간격 (중앙값 / 최대) | 16.7 / 45.9ms | 33.0 / 55.0ms | Debug는 걷는 내내 2~3프레임 간격이 반복 — 최적화 없는 SwiftUI·SceneKit 갱신 비용 |
| 전체화면 자동 일시정지 | 136.0~143.7초 (사용자 미니맵 탭) — gate `일시정지`, 누적·mesh 고정, 복귀 후 즉시 `누적 허용` | 122.6~124.7초, 동일 | 세션·메모리·격자 유지. 사용자 조작이라 연속 세션 판정에서 제외하지 않음 |
| 발열 | `상승`(8초) → `높음`(37초) | 시작부터 `높음` | 연속 촬영 조건. 수치는 이 발열 상태에서 얻은 값 |

Release의 DisplayLink 간격이 16.7ms를 넘은 구간은 셋뿐이다: 시작 직후 11~13초(39.3ms, 첫 mesh 출현),
37~40초(33.0ms, 발열 `상승` → `높음` 전환 시점), 140~142초(45.9ms, 전체화면 열기·닫기). 걷는 구간 자체에는
드롭이 없다. Debug의 p50 23.4ms는 09-05 재측정(12~14ms)의 약 2배인데, 두 편을 쉬지 않고 이어 촬영해 시작부터
발열 `높음`이었고 패널이 0.5초마다 SwiftUI 텍스트 십수 줄을 다시 그린다. 두 요인을 분리한 수치와 냉각 상태
재측정은 없다. 제출 빌드는 Release이므로 성능 예산 판정은 Release 열을 기준으로 한다. 패널의 `연속 누적 허용`
계수기는 전체화면 자동 일시정지에서 0으로 돌아가므로(최장 128.2초·114.0초) 세션 길이의 근거로 쓰지 않고,
녹화 전체에서 세션이 끊기지 않은 사실을 근거로 삼는다.
