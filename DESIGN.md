...
| 격자 UI 수신 지연 | 항상 0.0초 전 (매 처리 프레임 발행) | 판독 불가 | 스냅샷 경로에 적체 없음 |
| mesh 주기 · 지연 | 3초당 버전 +2 = 1.5초 주기 정확, 원본 프레임 → UI 도착 0.1초 | 판독 불가 | 타이머·hop 설계값 일치 |
| 유효점 → 격자 반영 | 2,500~3,072점 / 85~100% (먼 벽·천장 조준 시 126~800점) | 판독 불가 | stride 4 상한 도달 |
| 연속 누적 최장 | 71.3초 (시작 → 가림) | 판독 불가 | 가림 전까지 gate 개방 유지 |
...
| DisplayLink | 60.0Hz / 최대 간격 16.7ms (일시정지 중) | 판독 불가 | |
| 콜백 3초 창 p50 / p95 | 17.0 / 19.0ms (세대 1 스캔 마지막 3초) | 미측정 | 09-05 재측정(12~14 / 13~16ms)보다 높음. 발열 `높음` 상태에서 측정 |
| 메모리 | 448~452MB, 앱 실행 피크 593.6MB | 미측정 | 피크는 세대 0(2.37M점·6,035셀) 중 발생. 스캔 중 부하 탭을 안 봐 **추세 판정 불가** |
| 발열 | `높음`(serious) at 128초 | 미측정 | Prod 종료 → Dev 시작 간격 3분. 계측 조건 자체가 발열원 |

판독 공백: (1) 부하 탭을 일시정지 뒤에만 열어 스캔 중 p95·메모리 곡선이 영상에 없다. (2) 가림이
`insufficientFeatures` → `normal` 직행이라 `재인식 안정화` gate는 이 회차로 검증되지 않았다. (3) Production 수치는
패널이 없어 픽셀 판독뿐이다. 다음 회차는 걷기 중간에 부하 탭으로 전환해 끝까지 유지하고, 백그라운드 복귀로
relocalizing을 유도한다.
