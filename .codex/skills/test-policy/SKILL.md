...
- View·ViewModel은 테스트하지 않는다. `ScanViewModel.handle`이 이벤트 종류를 늘려 분기가 셋을 넘으면 그때 `internal`로 열고 테스트를 추가한다.
...
| 상태 머신 전이 (`ScanState`) | 권장 — 분기가 늘면 필수 | 없음 |
| 세션 lifecycle, 트래킹 경고, 권한 | 실기기 수동 | — |
| 레이아웃·버튼 배선, 상수, 단순 위임 | 제외 | — |

`scripts/check-structure.sh`는 세 Model 타입의 테스트 파일이 존재하는지, 테스트가 View·ViewModel을 참조하지 않는지 검사한다.
