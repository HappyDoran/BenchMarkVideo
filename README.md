...
| R2-4 오버레이 배치·전체화면 | ✅ | 우상단 170pt 오버레이 — 카메라 중앙 고정, 반경 3m. 배경은 정점 색 mesh의 직교 하향 실시간 렌더(1.5초 주기 갱신, 격자 비트맵보다 매끈), 마커·궤적은 기존 캔버스 정합. 탭하면 전체화면 2D/3D 뷰어 |
...
| 선택: 팬 / 줌 | 🔶 | 전체화면에서 핀치 줌(1~8배)·드래그 팬 — 오버레이 카메라 추종과 같은 변환 경로. 회전은 north-up 고정 원칙상 제외 (R2-5). 실기기 미검증 |
...
| 3D 재구성 뷰어 (+4, 상한 도달로 점수 외) | 🔶 전체화면 2D/3D 통합 뷰어 — ARKit mesh에 카메라 색을 정점 단위로 입힌 컬러 mesh(SceneKit), 회전·줌·팬 + 바닥 탭 측정(2D와 측정 상태 공유). 점군은 mesh 준비 전 fallback. 실기기 미검증 |
