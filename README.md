...
제출용 영상은 Production의 `ScreenRecording_09-06-2026 00-31-15_1.MP4`(124초)를 사용한다. 시작 → 같은 공간 이동 → 실시간 미니맵 누적 → 전체화면 2D/3D 결과 확인 → 초기화 흐름이 3분 안에 들어 있다. 저장소 용량을 늘리지 않기 위해 영상 파일은 커밋하지 않고 제출 이메일에 첨부하거나 외부 링크로 전달한다. Development 진단 영상은 수치 판독 근거이며 사용자용 데모에는 쓰지 않는다.
...
| 스캔 결과 내보내기 | ✅ 실기기 검증 — 공유 시트(iOS가 3D 항목 인식)·AirDrop 전송, .ply 검판(vertex 정합·격자 정렬·색) |
| 드리프트·정합 보정 시도 | ✅ 재로컬라이즈 복귀 후 1초 안정화 창 동안 누적·궤적 보류 (`DESIGN.md` 12절). 실기기 검증: 복귀 후 pts 696,499에서 1.5초+ 동결 → 재개, 격자 정합 유지(벽 한 겹) |
| 3D 재구성 뷰어 | ✅ 전체화면 2D/3D 통합 뷰어 — 정점 색 mesh(SceneKit) + 돌하우스 컬링(내부 투시), 회전·줌·팬, 바닥 탭 측정(2D와 측정 상태 공유 — 실기기 실증 1.34m/2.21m) |
...
- 콜백 처리(최종 빌드, 3초 창): p50 12~14ms / p95 13~16ms / max 20.8ms — 목표 <10ms는 Debug 기준 초과이나 시간 경과 저하 없이 평탄하고 전용 직렬 큐라 UI를 직접 막지 않는다. Production은 진단 코드가 없어 Release 콜백 수치는 미측정이다. 복셀 색 누적 추가 비용은 +1ms 수준.
...
- 3차 녹화 판독(09-06, 동일 동선 Dev 139초·Prod 124초): 시작 → 첫 격자·mesh 0.6초, 격자 UI 지연 0.0초, mesh 1.5초 주기·hop 0.1초, 연속 누적 71.3초, 렌즈 가림 중 누적 고정·복귀 후 유령 벽 없음, 전체화면 중 mesh 재빌드 스킵 실증. Production은 걷기 구간 고유 프레임 99.8%(드롭 흔적 없음), Development는 96.0%였다.
...
- Production 콜드 실행 별도 영상: 실행 애니메이션 시작 → 첫 카메라 프레임과 초기 미니맵 0.716초, 스캔 입력 → 첫 비어 있지 않은 격자 0.117초. 내부 FPS·메모리는 Production에서 의도적으로 계측하지 않는다. 전체 표와 한계는 `DESIGN.md` 10절.
...
- **갱신**: 제어 상태와 실제 누적 gate, 격자·mesh UI 수신 시각, mesh 원본 프레임 시각,
  누적 점·셀, 깊이 소스(smoothed/raw)와 최근 처리 프레임 시각, 연속 누적 허용 시간을 표시한다. `격자만 보기`로 같은 장면의 occupancy grid와
  mesh 표시 차이를 녹화할 수 있다.
- **부하**: `CADisplayLink`, 처리 콜백 약 0.5초 창 평균·최대와 3초 창 p50·p95, 누적 수신·깊이 처리
  프레임 수, mesh CPU 빌드, 메모리·피크, 앵커·정점·복셀 수와 발열 상태를 표시한다.
  패널에 없는 계측은 `frameCallback` signpost(Instruments Points of Interest)와 `perf` Logger
  콘솔 로그뿐이며, 두 값 모두 패널과 같은 원천을 쓴다.
...
| 성능 | R3 | `DESIGN.md` 10절 측정 — FPS ≥30, 콜백 <10ms, 3분 스캔 중 강제 종료·메모리 경고 없음 | 통과 — fps 60/59.9, 3분 생존·경고 없음. 콜백 p50 11.9ms(Debug)는 목표 초과이나 UI 격리로 영향 없음 (10절 해석). 09-06 재확인: DisplayLink 60.0Hz, Production 걷기 구간 프레임 드롭 없음. 콜백 p95 19.0ms(Debug·발열 높음), 메모리 추세는 미판정 |
| 녹화 진단 패널 | — | Development에서 패널 3탭 판독 가능, 격자만 보기 토글로 미니맵 배경 전환, 초기화 시 세대 증가 | 통과 (09-06) — 3탭과 스캔 중 부하 탭 판독, 격자·mesh 정합 일치, 세대 전환 확인. 미검증: `재인식 안정화` gate |
