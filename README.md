...
| 테스트 기기 | iPhone 15 Pro (iPhone16,1, LiDAR), iOS 26.6 — 2026-09-04 실기기 검증 완료 (화면 녹화 22편 프레임 판독), 2026-09-06 진단 패널 녹화 Development·Production 각 1편 재판독 |
...
- 3차 녹화 판독(09-06, 진단 패널, Dev·Prod 각 1편 약 2분): 시작 → 첫 격자·mesh 0.6초, 격자 UI 지연 0.0초, mesh 1.5초 주기·hop 0.1초, 연속 누적 71.3초, 렌즈 가림 9초 동안 누적 고정·복귀 후 유령 벽 없음, 전체화면 중 mesh 재빌드 스킵 실증. DisplayLink 60.0Hz/16.7ms. 콜백 p50 17.0 / p95 19.0ms (발열 `높음` 상태), 메모리 448MB·피크 593.6MB — 스캔 중 부하 탭을 안 봐 추세 미판정. Production은 걷기 구간 고유 프레임 99.8%(드롭 없음), Dev 96.0%. 표·공백은 `DESIGN.md` 10절.
...
결과 열: iPhone 15 Pro (iPhone16,1) × iOS 26.6, 2026-09-04 — 사용자 촬영 화면 녹화 22편을 1fps 프레임 추출로 판독한 회차. 세부 근거는 해당 일자 docs 커밋 본문. 2026-09-06 진단 패널 녹화(Development·Production 각 1편)로 스캔 시작·일시정지·초기화·실시간 갱신·전체화면·트래킹 경고·성능 행을 재확인했고, 결과는 아래 표와 같으며 수치는 `DESIGN.md` 10절 3차 판독 표에 있다.
...
| 녹화 진단 패널 | — | Development에서 패널 3탭 판독 가능, 격자만 보기 토글로 미니맵 배경 전환, 초기화 시 세대 증가 | 통과 (09-06) — 3탭 판독, 격자·mesh 정합 일치, 세대 0→1. 미검증: 스캔 중 부하 탭 판독, `재인식 안정화` gate |
